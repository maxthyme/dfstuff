...
if not df.viewscreen_layer_choose_language_namest:is_instance(dfhack.gui.getCurViewscreen()) then
    choices = df.viewscreen_setupadventurest:new() ; choices.page = 
    df.viewscreen_setupadventurest.T_page.Background ; 
    gui = 
    require 'gui' ; gui.simulateInput(choices, 'A_RANDOM_NAME') ; gui.simulateInput(choices, 'A_CUST_NAME')
end
